...
const no_throw: NoThrowT = NoThrow{_padding: MaybeUninit::uninit()};

extern"C"{
    
    pub fn _ZnwPvmRCSt6no_throw_t(sz: usize,nothrow: &NoThrowT) -> *mut core::ffi::c_void;
    pub fn _ZnwPvmmRCSt6no_throw_t(sz: usize,align: usize,nothrow: &NoThrowT) -> *mut core::ffi::c_void;
    ...
    fn allocate(&self,layout: Layout) -> Result<NonNull<[u8]>,AllocError>{
        ...
        if layout.size() == 0{
            ...
            if ptr.is_null(){
                ...
                Ok(unsafe{NonNull::new_unchecked(core::ptr::slice_from_raw_parts(ptr as *mut u8,layout.size()))})
            }
        }else{
            // SAFETY:
            // The C++ Standard Library prescribes no undefined behaviour for any global replaceable operator new
