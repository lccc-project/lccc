use crate::sema::{Declaration, FunctionSignature, Identifier, IntType, Program, Type};

use xlang::{abi::{self, option::Option::None as AbiNone}, ir};

fn identifier_to_path(id: Identifier) -> ir::Path {
    let Identifier::Basic(id) = id;
    ir::Path { components: abi::vec![ir::PathComponent::Text(abi::string::String::from(&id))] }
}
...
fn sig_to_fn_type(sig: FunctionSignature) -> ir::FnType {
    ir::FnType {
        ...
        params: sig.params.into_iter().map(|x| irgen_type(x)).collect::<Vec<ir::Type>>().into(),
        tag: ir::Abi::C,
    }
}
    ...
    for declaration in &program.declarations {
        ...
        file.root.members.insert(identifier_to_path(declaration.name().clone()), ir::ScopeMember {
            ...
            member_decl: match declaration {
                Declaration::Function { sig, .. } => ir::MemberDeclaration::Function(ir::FunctionDeclaration { ty: sig_to_fn_type(sig.clone()), body: AbiNone })
            },
