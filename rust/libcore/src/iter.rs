...
pub trait Iterator{
    ...
    fn step_by(self,by: usize) -> StepBy<Self> where Self: Sized{
        if by == 0{
    ...
    fn chain<U: IntoIterator<Item=Self::Item>>(self,other: U) -> Chain<Self,U::IntoIter> where Self: Sized{
        Chain{a: self,b: other}
    }

    fn zip<U: IntoIterator>(self,other: U) -> Zip<Self,U::IntoIter> where Self: Sized{
        Zip{a: self,b: other}
    }

    fn map<U,F: FnMut(Self::Item)->U>(self,op: F) -> Map<Self,F> where Self: Sized{
        Map{it: self,op}
    }

    fn for_each<F: FnMut(Self::Item)>(mut self,op: F) where Self: Sized{
        while let Some(v) = self.next(){
            op(v)
        }
    }

    fn filter<P: FnMut(&Self::Item)->bool>(self,pred: P) -> Filter<Self,P> where Self: Sized{
        Filter{it: self,pred}
    }

    fn filter_map<B,F: FnMut(Self::Item)->Option<B>>(self,op: F) -> FilterMap<Self,F> where Self: Sized{
        FilterMap{it: self,op}
    }

    fn enumerate(self) -> Enumerate<Self> where Self: Sized{
        Enumerate{it: self, cnt: 0usize}
    }
    ...
    fn skip_while<P: FnMut(&Self::Item)->bool>(self, pred: P)->SkipWhile<Self,P> where Self: Sized{
        SkipWhile{it: self,pred: Some(pred)}
    }

    fn take_while<P: FnMut(&Self::Item)->bool>(self, pred: P)->TakeWhile<Self,P> where Self: Sized{
        TakeWhile{it: self,pred,done: false}
    }
}
