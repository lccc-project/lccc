...
A simple block (non-loop, non-if, non-match block expression with a single constitutent expression, and no statements other than items) is mangled as the constituent expression.
