...
* Otherwise, each point that requires the generation of a destructor generates a unique destructor (up to one per translation unit), and each generation is then merged into a single instance.
