...
If a program links `std`, `test`, or `proc_macro`, there shall be a static defined with weak linkage, which is the mangled form of the static `alloc::alloc::__global_allocator`. It shall have type `&'static dyn alloc::alloc::GlobalAllocator`
...
The following symbols shall be defined by the implementation, when any panic is permitted to unwind. All functions in this section are defined `extern "Rust"`.

* `std::panicking::lcrust::begin_unwind(*mut PanicUnwindInformation)`, which returns `!`. 
    * Begins an unwind according to this abi. Called by the default panic handler when unwinding.
* `std::panicking::lcrust::begin_catch_native(*mut PanicUnwindInformation)`, which returns `*mut dyn Any`.
    * Called when catching a native (lcrust) execption. Allocates a new buffer using the global allocator, copies the panic object into the buffer, and returns it as the appropriate fat pointer. Then deallocates the except with `std::panic::__panic_deallocate`. Decrements the thread-local panic counter.
...
* `std::panicking::lcrust::dispose_foreign_exception(*mut ForeignExceptionType)` which returns `()`.
    * Called by the destructor of `ForeignPanic`. Destroys and deallocates the exception by using a platform and unwind-dependent method. If the destructor of the foreign exception throws an exception, the implementation shall abort.
...
* `std::panic::__deallocate_exception(*mut PanicUnwindInformation)`
